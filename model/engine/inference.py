...
def inference_for_ss(cfg, model, test_loader):
    ...
    if args.test_aiu:
        ...
        threshold_map = torch.Tensor(thresholds).view(len(thresholds),
                                                      1, 1).to('cuda')
    ...
    model.eval()
    for iteration, (
            imgs, sr_targets, masks, kernel_targets, fname, img_unfold_shape,
            seg_unfold_shape) in enumerate(test_loader, 1):
        fnames += list(fname)
        ...
        damy_kernel = torch.zeros((num_batch * num_patch, 1,
                                   cfg.BLUR.KERNEL_SIZE, cfg.BLUR.KERNEL_SIZE))

        sr_preds, segment_preds, kernel_preds = model(imgs, damy_kernel,
...
def inference_tti_building(args, cfg, model, test_loader):
    ...
    model.eval()
    for iteration, (_imgs, fname, img_unfold_shape,
                    seg_unfold_shape) in enumerate(test_loader, 1):
        ...
        for imgs in _imgs:
            num_patch = img_unfold_shape[0][2] * img_unfold_shape[0][3]
            damy_kernel = torch.zeros((num_batch * num_patch, 1,
                                       cfg.BLUR.KERNEL_SIZE,
                                       cfg.BLUR.KERNEL_SIZE))
            sr_pred, segment_pred, kernel_pred = model(imgs, damy_kernel)
            if 'sr_preds' in locals():
                sr_preds = torch.cat((locals()['sr_preds'], sr_pred), 0)
                segment_preds = torch.cat((locals()['segment_preds'],
                                           segment_pred), 0)
                kernel_preds = torch.cat((locals()['kernel_preds'],
                                          kernel_pred), 0)
            else:
        ...
        sr_preds = joint_patch(sr_preds, img_unfold_shape[0],
                               batch_size=len(img_unfold_shape))
        segment_preds = joint_patch(segment_preds, seg_unfold_shape[0],
                                    batch_size=len(seg_unfold_shape))
    ...
    for i in range(batch_size):
        ...
        for j in range(num_th):
            ...
            surface_distances = surface_distance.compute_surface_distances(
                gt, pred, spacing_mm=(vertical, horizontal))
            ...
            if not distances_gt_to_pred and not distances_pred_to_gt:
                hd_scores[i, j] = 0
            elif not distances_gt_to_pred or not distances_pred_to_gt:
                hd_scores[i, j] = max_img_len
                num_hd_outliner += 1
                if num_hd_outliner % 100 == 0:
                    print(f'hd score is outlire (num:{num_hd_outliner})')
            else:
                hd_scores[i, j] = surface_distance.compute_robust_hausdorff(
                    surface_distances, percentile)
