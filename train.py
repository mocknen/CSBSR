...
def train(args, cfg):
    ...
    if cfg.DATASET.ONLY_IMAGES:
        trainval_dataset = SRPretrainDataSet(cfg, cfg.DATASET.TRAIN_IMAGE_DIR,
                                             transforms=train_transforms,
                                             sr_transforms=sr_transforms)
    elif 'RetinalSeg' in cfg.DATASET.TRAIN_IMAGE_DIR:
        trainval_dataset = RetinalDataSet(cfg, cfg.DATASET.TRAIN_IMAGE_DIR,
                                          cfg.DATASET.TRAIN_MASK_DIR,
                                          transforms=train_transforms,
                                          sr_transforms=sr_transforms)
    else:
        trainval_dataset = CrackDataSet(cfg, cfg.DATASET.TRAIN_IMAGE_DIR,
                                        cfg.DATASET.TRAIN_MASK_DIR,
                                        transforms=train_transforms,
                                        sr_transforms=sr_transforms)
    ...
    eval_batch_sampler = BatchSampler(sampler=eval_sampler,
                                      batch_size=cfg.SOLVER.BATCH_SIZE,
                                      drop_last=False)
    eval_loader = DataLoader(val_dataset, num_workers=1, # num_workers = 1 for memory retrenchment
    ...
    if cfg.DATASET.ONLY_IMAGES and cfg.MODEL.SR != 'DSRL':
        ...
        if cfg.MODEL.SR_SEG_INV:
            model = JonitInvModelWithLoss(
                cfg, num_train_ds=train_size, resume_iter=args.resume_iter,
                sr_transforms=sr_transforms).to(device)
            # print(f'------------Model Architecture-------------\n\n<Network SS>\n{model.segmentation_model}\n\n<Network SR>\n{model.sr_model}')
        else:
            model = JointModelWithLoss(cfg,
                                       num_train_ds=train_size,
    ...
    if cfg.MODEL.OPTIMIZER == 'Adam':
        optimizer = torch.optim.Adam(model.parameters(), lr=cfg.SOLVER.LR,
                                     betas=(0.9, 0.999), eps=1e-8) # betas and eps is referenced from KBPN.
    elif cfg.MODEL.OPTIMIZER == 'SGD':
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad,
                                           model.parameters()),
                                    lr=cfg.SOLVER.LR, momentum=0.9,
                                    weight_decay=5e-4)

    sch_func = UpDownScheduler(cfg.SOLVER.SR_PRETRAIN_ITER[1],
                               args.resume_iter, scheduler_flag)
    scheduler = LambdaLR(optimizer, lr_lambda = sch_func)

    if args.resume_iter > 0:
        print('Resume from {}'.format(os.path.join(
            cfg.OUTPUT_DIR, 'model',
            'iteration_{}.pth'.format(args.resume_iter))))
    ...
    if cfg.DATASET.ONLY_IMAGES:
        do_pretrain_sr(args, cfg, model, optimizer, scheduler, train_loader,
                       eval_loader)
    else:
        do_train(args, cfg, model, optimizer, scheduler, train_loader,
                 eval_loader)
...
def main():
    ...
    parser.add_argument('--config_file', type=str,
                        default='./config/configs_train.yaml', metavar='FILE',
    ...
    parser.add_argument('--wandb_prj_name', type=str, default='CSBSR')
    ...
    if not args.debug and args.resume_iter == 0:
        ...
        shutil.copy2(args.config_file, os.path.join(cfg.OUTPUT_DIR,
                                                    'config.yaml'))
